...
function SessionDialog({ props }) { // {selectedSession, onClose, course_name}
  ...
  useEffect(() => {
    if (props.selectedSession) { // VIEW SESSION
  ...
  useEffect(() => {
    async function fetchTopics() {
      try {
        const response = await axios.get(`${import.meta.env.VITE_APP_HOST2}/topics/all/${props.course_name}`, {
  ...
  const handleDeleteSession = async (sessionId) => {
    try {
      const response = await axios.delete(`${import.meta.env.VITE_APP_HOST2}/session/delete/${sessionId}`, {
        headers: { Authorization: `Bearer ${authToken}` },
      });
      props.setSessions(props.sessions.filter(session => session.session_id !== sessionId));
  ...
  const handleAddOrUpdateSession = async () => {
    try {
      if (!isEditMode) {
        const response = await axios.post(`${import.meta.env.VITE_APP_HOST2}/session/bulkCreate`, {
          sessions: [
            {
        ...
        const response = await axios.put(`${import.meta.env.VITE_APP_HOST2}/session/bulkUpdate`, {
          sessions: [
            {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end mt-6">
            {isEditMode && (
              <button
                type="button"
                onClick={() => handleDeleteSession(props.selectedSession?.session_id)}
